...
| 「学习记录」「/kb learn <内容>」 | `vault_save`（默认 type: concept, status: draft） |
...
2. **确定元数据**：
   ...
   - `status` — 默认 `draft`，成熟后提升为 `review` → `permanent`，废弃笔记设为 `archived`
...
**规则：**
1. 第一层：`graphify_query` + `graphify_status` — 检查图谱版本，定位符号归属与调用链
2. 第二层：`vault_search(graphify/ + permanent/)` — 模块职责和历史方案
...
- 搜索范围覆盖 permanent/ + project/ + graphify/ 全部笔记
